...
async def test_compaction_and_union_read(session) -> None:
    ...
    await _seed(session, "BTC", old_day + timedelta(hours=3),
               score_sum=0.5, conf_sum=0.5, wsum=0.25, n=1)
    await _seed(session, "BTC", old_day + timedelta(hours=9),
               score_sum=0.5, conf_sum=0.5, wsum=0.25, n=1)
    # a recent hourly bucket that must survive compaction
    await _seed(session, "BTC", now - timedelta(hours=1),
               score_sum=-1.0, conf_sum=1.0, wsum=-1.0, n=1)
