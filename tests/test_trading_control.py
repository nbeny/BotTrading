...
def _handler(cache):
    ...
    return control.ControlHandler(
        cache, engine=None, kraken=None, defaults=config.TradingConfig()
    )
...
def test_set_kill_switch() -> None:
    cache = FakeCache()
    asyncio.run(
        _handler(cache).handle(_cmd(ControlCommand.SET_KILL_SWITCH, enabled=False))
    )
...
def test_set_auto_trading() -> None:
    cache = FakeCache()
    asyncio.run(
        _handler(cache).handle(_cmd(ControlCommand.SET_AUTO_TRADING, enabled=False))
    )
...
def test_set_caps_partial() -> None:
    cache = FakeCache(
        values={"trading:runtime": {"max_order_usd": 500.0, "max_leverage": 3.0}}
    )
    asyncio.run(
        _handler(cache).handle(_cmd(ControlCommand.SET_CAPS, max_order_usd=250.0))
    )
...
class FakeEngine:
    ...
    async def adjust_sltp(
        self, event_id, *, stop_loss=None, take_profit=None, issued_by=None
    ):
...
def _handler_with_engine(cache, engine):
    ...
    config = load_module("config")
    return control.ControlHandler(
        cache, engine=engine, kraken=None, defaults=config.TradingConfig()
    )
...
def test_close_position_dispatches_to_engine() -> None:
    engine = FakeEngine()
    asyncio.run(
        _handler_with_engine(FakeCache(), engine).handle(
...
def test_approve_reject_dispatch() -> None:
    ...
    class E:
        ...
        async def reject_opportunity(
            self, event_id, *, reason="operator_reject", issued_by=None
        ):
            self.calls.append(("reject", event_id, reason, issued_by))

    e = E()
    h = control.ControlHandler(
        FakeCache(), engine=e, kraken=None, defaults=config.TradingConfig()
    )
    asyncio.run(h.handle(_cmd(ControlCommand.APPROVE_OPPORTUNITY, event_id="e1")))
    asyncio.run(
        h.handle(_cmd(ControlCommand.REJECT_OPPORTUNITY, event_id="e2", reason="no"))
    )
...
def test_manual_order_dispatch() -> None:
    ...
    class E:
        ...
        async def manual_order(
            self, *, symbol, side, order_type, quantity, price=None, issued_by=None
        ):
