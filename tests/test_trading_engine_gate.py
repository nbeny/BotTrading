...
from tests.test_trading_engine import FakeCache, FakeKraken, FakeProducer, _signal
...
def test_auto_off_queues_pending_no_orders() -> None:
    cache = FakeCache(
        values={
            "trading:runtime": {"auto_trading_enabled": False, "trading_enabled": True}
        }
    )
    ...
    assert (
        cache._values.get("trading:pending:" + _signal().event_id) is None
    )  # different id
...
def test_auto_on_executes() -> None:
    cache = FakeCache(
        values={
            "trading:runtime": {"auto_trading_enabled": True, "trading_enabled": True}
        }
    )
    producer, kraken = FakeProducer(), FakeKraken()
    engine = _engine(cache, producer, kraken)
